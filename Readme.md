# 🚀 My Personal Portfolio
A responsive portfolio website built to showcase my projects and technical skills.

## 🛠 Key Features
*   **Neo-Brutalist Design** – High-impact UI built with [Tailwind CSS 4.0](https://tailwindcss.com) featuring bold borders and heavy shadows.
*   **High-Performance Search** – Direct [GIPHY API](https://developers.giphy.com) integration for sub-second asset delivery.
*   **Smart Debouncing** – Optimized JS logic to minimize API overhead and improve performance.
*   **One-Click Copy** – Integrated [Clipboard API](https://developer.mozilla.org) for instant URL sharing.
*   **Responsive Masonry Grid** – Fluid layout that scales from mobile to ultra-wide displays.
*   **Brand-Safe Content** – Hardcoded `rating=g` filter for professional environments.

## 💻 Tech Stack
*   **Logic:** Vanilla JavaScript (ES6+)
*   **Styling:** Tailwind CSS 4.0
*   **Data Source:** GIPHY API
*   **Typography:** [Poppins (Google Fonts)](https://fonts.google.com)

## 📬 Contact Me
*   **GitHub**: [@goldylox752](https://github.com)
*   **LinkedIn**: [Byron Sanche](https://www.linkedin.com)

## 📈 Future Roadmap
*   [ ] **Category Tags**: Quick-search buttons for popular brands (e.g., Tech, Fashion).
*   [ ] **Dark Mode**: High-contrast theme toggle for the neo-brutalist layout.
*   [ ] **Download Assets**: Direct local saving of GIF files.

<div align="center">
  <a href="https://github.com">
    <img src="https://programing-gifs.vercel.app/" width="400" height="auto" alt="Programming Gifs">
  </a>
</div>


  

