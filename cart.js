...
function renderCart() {
  ...
  if(cart.length === 0){
    ...
    document.getElementById("checkout-stripe-btn").style.display = "none";
    document.getElementById("checkout-btc-btn").style.display = "none";
  ...
  document.getElementById("checkout-stripe-btn").style.display = "inline-block";
  document.getElementById("checkout-btc-btn").style.display = "inline-block";
}

// Multi-item Stripe checkout
document.getElementById("checkout-stripe-btn").onclick = async () => {
...
// Dynamic BTC checkout
document.getElementById("checkout-btc-btn").onclick = async () => {
  if(cart.length === 0) return alert("Cart is empty!");
  let totalCAD = cart.reduce((sum, item) => sum + item.priceCAD, 0);

  // Fetch BTC rate in CAD from CoinGecko API
  ...
  const totalBTC = (totalCAD / btcRate).toFixed(8); // 8 decimal places
